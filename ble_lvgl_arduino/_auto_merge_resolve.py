...
OUTPUT_FILE = r"c:\Users\xx.mu\.trae-cn\worktrees\ble_lvgl_arduino\_auto_merge_result.txt"
...
def resolve_conflict_in_content(content, file_path):
    ...
    1. Find all conflict blocks (<<<<<<< ... ======= ... >>>>>>>)
    2. For each block, try to intelligently merge both sides
    3. Strategy:
       - If both sides have different lines, combine them (deduplicate)
       - For function/variable declarations, prefer the more complete version
       - For comments/docs, combine
       - If truly conflicting, keep source branch (HEAD) version as base
         but try to include target branch additions
    """
    pattern = re.compile(
        r'<<<<<<<[^\n]*\n(.*?)=======\n(.*?)>>>>>>>[^\n]*\n',
        re.DOTALL
    ...
    def replace_conflict(match):
        ...
        src_content = match.group(1)  # HEAD (source branch)
        tgt_content = match.group(2)  # master (target branch)
...
def main():
    log("=" * 60)
    log("AUTO MERGE + CONFLICT RESOLUTION SCRIPT")
    log("=" * 60)
